...
function sanitizeFolderName(name: string) {
  const cleaned = name
    ...
    .replace(/\s+/g, ' ')
    .trim()
    .replace(/[.\s]+$/g, ''); // trailing dots/spaces are invalid on Windows
...
const [playlistCardBusyId, setPlaylistCardBusyId] = useState<string | null>(null);
const [reviewTargetDir, setReviewTargetDir] = useState<DirectoryHandle | null>(null);
  ...
  async function ensureLibraryRoot(): Promise<DirectoryHandle> {
  ...
  async function selectPlaylistCard(playlist: YouTubePlaylist) {
    setPlaylistPickerError('');
    setPlaylistCardBusyId(playlist.id);
    try {
      // The library root is asked for once (or reused/reconnected); a
      // dedicated subfolder named after the playlist is created inside it
      // so each playlist stays organized in its own folder.
      const root = await ensureLibraryRoot();
      const folderName = sanitizeFolderName(playlist.title);
      const subDir = await root.getDirectoryHandle(folderName, { create: true });

      setPlaylistPickerOpen(false);
      setPlaylistSearch('');
      setSyncUrl(playlist.url);
      await analyzeAndOpenReview(subDir, playlist.url);
    } catch (err) {
      setPlaylistPickerError(err instanceof Error ? err.message : 'Unable to create a folder for this playlist.');
    } finally {
      setPlaylistCardBusyId(null);
    }
  }
  ...
  async function confirmReviewDownload() {
    if (!reviewData || !reviewTargetDir) return;
    ...
    try {
      for (let i = 0; i < toDownload.length; i++) {
        ...
        await saveToDirectory(reviewTargetDir, name, blob);
...
{playlistPickerOpen && (
  ...
  >
    <div className="modalCard playlistPickerCard">
      ...
      <div className="modalHeader">
        <div>
          ...
          <div className="modalSub">
            ...
            {youtubePlaylists.length === 1 ? '' : 's'} found · tap one to sync it into its own folder
          </div>
        </div>
      ...
      <div className="playlistCardGrid">
        {youtubePlaylists
          .filter(playlist =>
            playlist.title
              .toLowerCase()
          ...
          .map(playlist => {
            const busy = playlistCardBusyId === playlist.id;
            ...
            return (
              <button
                ...
                onClick={() => selectPlaylistCard(playlist)}
                disabled={playlistCardBusyId !== null}
              >
                {playlist.thumbnail ? (
                  <img
                    className="playlistCardThumb"
                    src={playlist.thumbnail}
                    alt=""
                  />
                ) : (
                  <div className="playlistCardThumb" />
                )}

                <span className="playlistCardTitle">
                  {playlist.title}
                </span>

                <span className="playlistCardMeta">
                ...
                {busy && (
                  <span className="playlistCardBusy">
                    Preparing folder…
                  </span>
                )}
              </button>
      ...
      <div className="syncNote">
        Selecting a playlist creates (or reuses) a folder named after it inside your chosen audio library folder, then opens Sync Review for that folder.
      </div>
